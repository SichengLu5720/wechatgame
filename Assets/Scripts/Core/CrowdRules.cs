...
namespace StairsCrowd.Core
{
    ...
    public sealed class Board
    {
        ...
        public Move TryMove(int from,int to,int[] actorSlots=null){var m=Rules.Preview(Level,Current,from,to);if(!m.ok)return m;var next=Rules.Apply(Current,m);if(actorSlots==null)next.actorSlots=null;if(actorSlots!=null){if(actorSlots.Length!=Level.groups.Length*4)throw new ArgumentException("Invalid actor slots");for(int n=0;n<Level.nodes.Length;n++)if(Level.nodes[n].transit){var used=new HashSet<int>();foreach(int g in next.queues[n])for(int i=0;i<4;i++){int slot=actorSlots[g*4+i];if(slot<0||slot>=next.queues[n].Count*4||!used.Add(slot))throw new ArgumentException("Invalid transit slot assignment");}}next.actorSlots=(int[])actorSlots.Clone();}Save();Current=next;Moves++;return m;}
